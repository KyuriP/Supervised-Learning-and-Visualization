...
imp <- mice(sampled.housing, m = 5, seed = 1)
```

```{r}
# model based on backward elimination
fit <- with(imp, lm(price ~  mq + n_bathrooms + year_of_construction + 
                    energy_class + heating + has_pool + has_elevator))
...
```{r}
# model based on majority method
scope <- list(upper = ~ n_rooms + floor + mq + n_bathrooms + year_of_construction + 
                      energy_class + heating + has_garage + has_terrace + has_garden + 
...
# actual model
fit <- with(imp, lm(price ~  energy_class + has_alarm + has_elevator + has_garden +
                    has_pool + heating + is_furnished + mq + n_bathrooms + 
                    n_rooms + year_of_construction))
...
```{r}
# stacked
test <- cbind(complete(imp, "long"), weights = 1)

summary(lm(price ~ energy_class + has_alarm + has_elevator + has_garden +
                    has_pool + heating + is_furnished + mq + n_bathrooms + 
                    n_rooms + year_of_construction, weights = weights, data = test))
```
