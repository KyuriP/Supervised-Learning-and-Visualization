...
price_by_reg <- housing_red %>% group_by(region) %>% 
  summarize(mean = mean(price, na.rm=T), median = median(price, na.rm=T), variance = var(price, na.rm=T)) %>% left_join(.,reg_2022, by = c("region" = "DEN_REG")) %>% st_as_sf()
```

```{r price_by_prov}
price_by_prov <- housing_red %>% group_by(province) %>% 
  summarize(avg_price = mean(price, na.rm=T), median = median(price, na.rm=T), variance = var(price, na.rm=T)) %>% left_join(.,prov_2022, by = c("province" = "DEN_PROV")) %>% st_as_sf()
```

### Plot

We inspect the average housing price by region. 
